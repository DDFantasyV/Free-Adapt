...
class AdversarialTrainer:
    def __init__(self, arch: str = "resnet18", num_classes: int = 10, topdir: str = "fw_experiments"):
        ...
        if torch.cuda.device_count() >= 1:
            ...
            sys.exit("CPU Training is not allowed.")
    ...
    def train(
        ...
        """
        assert mode in ["pgd", "standard", "fgsm_adapt", "grad_align", "free", "free_adapt"]
        ...
        for epoch in range(n_epochs):
            ...
            if (epoch == 0) or ((epoch + 1) % eval_freq == 0):
                ...
                if mode == "free_adapt":
                    if epoch == 0:
                        pre_clean_acc = clean_acc_eval
                        pre_adv_acc = acc_eval
                    else:
                        cur_clean_acc = clean_acc_eval
                        cur_adv_acc = acc_eval
                        if cur_adv_acc + cur_clean_acc - pre_adv_acc - pre_clean_acc <= 0:
                            if cur_clean_acc - pre_clean_acc <= 0:
                                mb = mb - 1
                                print("Clean Acc descended, change m to ", mb)
                            if cur_adv_acc - pre_adv_acc <= 0:
                                mb = mb + 1
                                print("Adv Acc descended, change m to ", mb)
                        pre_clean_acc = cur_clean_acc
                        pre_adv_acc = cur_adv_acc
